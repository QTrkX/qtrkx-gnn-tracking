...
def batch_train_step(n_step):
    with tf.GradientTape() as tape:
        for batch in range(config['batch_size']):
            X, Ri, Ro, y = train_data[train_list[n_step*config['batch_size']+batch]]
...
if __name__ == '__main__':
    ...
    if config['network'] == 'GNN':
        ...
        GNN.config = config
    elif config['network'] == 'GNN2':
        from qnetworks.QGNN2 import GNN
        GNN.config = config
    elif config['network'] == 'HepTrkXQGNN':
        from qnetworks.HepTrkXQGNN import GNN
